...
class Ticket{
    ...
    function load($id) {
        ...
        if(($res=db_query($sql)) && db_num_rows($res)):
            ...
            $this->sow = $row['sow'];
            $this->travel = $row['travel'];
            $this->travelRate = $row['travelrate'];
            /* End MOD1 changes */
    ...
    function getCsv(){
        $csv = sprintf('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s',
                        $this->id,$this->extid,$this->email,$this->status,$this->created,$this->updated,$this->lastrespdate,$this->lastmsgdate,$this->duedate,$this->priority,$this->priority_id,
                        $this->fullname,$this->staff_id,$this->dept_id,$this->topic_id,$this->dept_name,$this->subject,$this->helptopic,$this->overdue,$this->siteName,$this->siteAddress,
                        $this->csz,$this->contactName,$this->contactPhone,$this->contactEmail,$this->sow,$this->travel,$this->travelRate);
        return $csv;
    }
        ...
        if($var['topicId'] && ($topic= new Topic($var['topicId'])) && $topic->getId()) {
         ...
         if(!$errors){
             $sql='UPDATE '.TICKET_TABLE.' SET updated=NOW() '.
                    ...
                    ',siteName='.db_input($var['siteName']).
                    ',siteAddress='.db_input($var['siteAddress']).
                    ',csz='.db_input($var['csz']).
                    ',contactName='.db_input($var['contactName']).
                    ',contactPhone='.db_input($var['contactPhone']).
                    ',contactEmail='.db_input($var['contactEmail']).
                    ',sow='.db_input($var['sow']).
                    ',travel='.db_input($var['travel']).
                    ',travelrate='.db_input($var['travelrate']).
       ...
       /* Coders never code so fully and joyfully as when they do it for free  - Peter Rotich */
        ...
        if(db_query($sql) && ($id=db_insert_id())){
            ...
            if(!$cfg->useRandomIds()){
                ...
                $curDate=date("y");
                $extId=$curDate.$id; //To make things really easy we are going to use autoincrement ticket_id.
