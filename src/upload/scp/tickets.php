...
if(!$errors && ($id=$_REQUEST['id']?$_REQUEST['id']:$_POST['ticket_id']) && is_numeric($id)) {
    ...
    if(!$errors && $_REQUEST['a']=='print'){
        $page='printticket.inc.php';
    }
...
if($stats['overdue']) {
    ...
    if(!$sysnotice && $stats['overdue'] > 10 && $_REQUEST['a'] != 'print')
...
//MOD1 - Print Modification
if($ticket){
    $nav->addSubMenu(array('desc'=>'Print Ticket','href'=>'tickets.php?a=print&id='.$ticket->getId(),'iconclass'=>'printTicket'));
}
//End MOD1
...
$title = 'Trident OSTicket';
if($_REQUEST && $_REQUEST['a'] == 'print'){
    $title = $ticket->getExtId().' - '.$ticket->getSiteName();
}
