...
if($_POST && $_REQUEST['t'] && !$errors):
    ...
    switch(strtolower($_REQUEST['t'])):
        ...
        case 'api':
            ...
            break;
        case 'staffapi':
            include_once(INCLUDE_DIR.'class.staffapi.php');
            switch(strtolower($_POST['do'])) {
                case 'add':
                    if(StaffApi::add($_POST['username'], $errors))
                        $msg='Key created successfully for '.$_POST['username'];
                    elseif(!$errors['err'])
                        $errors['err']='Error adding username: '.$_POST['username'];
                    break;
		case 'mass_process':
                    if(!$_POST['ids'] || !is_array($_POST['ids'])) {
                        $errors['err']='You must select at least one entry to process';
                        ...
                        $ids=implode(',',$_POST['ids']);
                        if($_POST['enable'] || $_POST['disable']) {
                            $resp=db_query('UPDATE '.STAFF_API_TABLE.' SET status='.db_input($_POST['enable']?1:0).' WHERE id IN ('.$ids.')');
                            ...
                            $resp=db_query('DELETE FROM '.STAFF_API_TABLE.'  WHERE id IN ('.$ids.')');
                            if($resp && ($i=db_affected_rows())){
                                $msg="$i of $count selected key(s) deleted";
                            }else{
                                $errors['err']='Unable to delete selected key(s). Try again';
...
switch($thistab){
    ...
    case 'staffapi':
        ...
        $nav->addSubMenu(array('desc'=>'Staff API','href'=>'admin.php?t=staffapi','iconclass'=>'api'));
        switch($thistab):
            case 'settings':            
            case 'pref':        
